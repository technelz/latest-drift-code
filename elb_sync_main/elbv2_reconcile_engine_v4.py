...
def _discover_subnets_with_traits(env: Environment) -> Dict[str, Dict[str, Any]]:
    ...
    for page in paginator.paginate(Filters=[{"Name": "vpc-id", "Values": [env.vpc_id]}]):
        for subnet in page.get("Subnets", []):
            ...
            subnets[subnet_id]["_Name"] = _subnet_name(subnet)
            subnets[subnet_id]["_NormalizedName"] = normalize_name_for_match(_subnet_name(subnet))
    ...
    for subnet_id, subnet in subnets.items():
        ...
        subnet["_Public"] = any(_route_has_internet_gateway(route) for route in routes)
        subnet["_Routed"] = any(
            route.get("DestinationCidrBlock") == "0.0.0.0/0" and _route_has_nat_or_gateway(route)
            for route in routes
        )
...
def _discover_security_groups_with_traits(env: Environment) -> Dict[str, Dict[str, Any]]:
    ...
    for page in paginator.paginate(Filters=[{"Name": "vpc-id", "Values": [env.vpc_id]}]):
        for sg in page.get("SecurityGroups", []):
            ...
            groups[group_id]["_Name"] = _security_group_name(sg)
            groups[group_id]["_NormalizedName"] = normalize_name_for_match(_security_group_name(sg))
            groups[group_id]["_NormalizedGroupName"] = normalize_name_for_match(sg.get("GroupName", ""))
    return groups


def _select_single_candidate(candidates: List[str], label: str, source_id: str, resource_name: str) -> str:
    unique = sorted(set(candidates))
    if len(unique) == 1:
        return unique[0]
    if not unique:
        raise RuntimeError(f"No unambiguous target {label} match for source {label} {source_id} on {resource_name}")
    raise RuntimeError(
        f"Ambiguous target {label} match for source {label} {source_id} on {resource_name}: "
        f"{', '.join(unique)}"
    )
...
def _auto_match_subnet_id(
    ...
    src_norm = src.get("_NormalizedName", "")
    src_mask = src.get("_Mask")
    src_public = bool(src.get("_Public"))
    src_routed = bool(src.get("_Routed"))

    # Strongest match: normalized Name tag, with prod/dr/dev/stage tokens removed.
    if src_norm:
        candidates = [sid for sid, subnet in target_subnets.items() if subnet.get("_NormalizedName") == src_norm]
        if candidates:
            return _select_single_candidate(candidates, "subnet", source_subnet_id, resource_name)

    # Safe fallback: same route-table behavior and same CIDR mask. This is only safe
    # when it resolves to exactly one target subnet.
    candidates = [
        sid for sid, subnet in target_subnets.items()
        if subnet.get("_Public") == src_public
        and subnet.get("_Routed") == src_routed
        and subnet.get("_Mask") == src_mask
    ]
    return _select_single_candidate(candidates, "subnet", source_subnet_id, resource_name)
...
def _auto_match_security_group_id(
    ...
    normalized_values = {
        src.get("_NormalizedName", ""),
        src.get("_NormalizedGroupName", ""),
    }
    normalized_values = {v for v in normalized_values if v}

    candidates = [
        gid for gid, sg in target_groups.items()
        if sg.get("_NormalizedName") in normalized_values
        or sg.get("_NormalizedGroupName") in normalized_values
    ]
    return _select_single_candidate(candidates, "security group", source_sg_id, resource_name)
...
def resolve_subnet_ids_for_load_balancer(
    ...
    if missing:
        ...
        for src_id in missing:
            mapped.append(_auto_match_subnet_id(src_id, source_subnets, target_subnets, desired.name))
