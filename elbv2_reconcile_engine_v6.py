...
def _score_subnet_candidate(source_subnet: Dict[str, Any], target_subnet: Dict[str, Any]) -> Tuple[int, List[str]]:
    score = 0
    reasons: List[str] = []

    if source_subnet.get("AvailabilityZoneId") and source_subnet.get("AvailabilityZoneId") == target_subnet.get("AvailabilityZoneId"):
        score += 100
        reasons.append("same-az-id")
    ...
    # Prefer subnets with enough available IPs for LB ENIs, but keep this as a weak
    # tie-breaker because source/target capacity may intentionally differ.
...
def _select_best_scored_candidate(
    ...
    if not scored or scored[0][1] < minimum_score:
        raise RuntimeError(
            ...
            f"Best score={scored[0][1] if scored else 'none'}, required>={minimum_score}."
        )
...
def _auto_match_subnet_id(
    ...
    for target_id, target_subnet in target_subnets.items():
        if target_id in selected:
            continue
        score, reasons = _score_subnet_candidate(src, target_subnet)
...
def resolve_subnet_ids_for_load_balancer(
    ...
    if missing:
        ...
        for src_id in missing:
            target_id = _auto_match_subnet_id(src_id, source_subnets, target_subnets, desired.name, selected)
            mapped.append(target_id)
...
def parse_args() -> Args:
    parser = argparse.ArgumentParser(description="Enterprise AWS ELBv2 DR Routing Reconciliation Engine v6")
...
def main() -> int:
    ...
    try:
        log("\n============================================================")
        log(" Enterprise AWS ELBv2 DR Routing Reconciliation Engine v6")
        log("============================================================")
